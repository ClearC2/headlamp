...
var _default = function _default(app) {
  ...
  app.get('/_grep', function (req, res) {
    ...
    if (q.length < minChars) {
      return res.status(400).json({ error: 'Minimum character limit: ' + minChars + '.' });
    }
    ...
    if (q.includes('"') && q.includes("'")) {
      return res.status(400).json({ error: 'Searches cannot include both single and double quotes. grep limitation.' });
    }
    ...
    srcDirs.forEach(function (dir) {
      var searchResult = '';
      var sourceJS = _path2.default.join(dir, '*.js');
      var sourceJSON = _path2.default.join(dir, '*.json');
      try {
        var cmd = 'git grep -nF --untracked ' + escChar + q + escChar + ' -- \'' + sourceJS + '\' \'' + sourceJSON + '\'';
...
function getLastModified(file) {
  try {
    ...
    file = _path2.default.basename(file);
...
function findPathInSrc(options, testPath) {
  ...
  var parts = testPath.split('/').filter(function (p) {
    return !!p && !p.includes(':');
  });
  // no parts to search
  if (parts.length === 0) {
  ...
  srcDirs.forEach(function (dir) {
    ...
    var sourceJS = _path2.default.join(dir, '*.js');
    var sourceJSON = _path2.default.join(dir, '*.json');
    var partsPattern = ['git grep -n --untracked \'' + first + '\' -- \'' + sourceJS + '\' \'' + sourceJSON + '\''];
    rest.forEach(function (p) {
      return partsPattern.push('git grep \'' + p + '\'');
    });
